...
public class JobDAO {
     ...
     * @return An <code>ArrayList</code> of <code>QueriedApplicationDTO</code>
     * objects
     */
    ...
    /**
     * Updates an applicants status to whatever data given by the parameter
     * <code>value</code>.
     *
     * @param personID ID of the applicant to be updated
     * @param value True of false, depending on whether the applicant is
     * employed or fired
     ...
     * @param languageCode Language code
     * @return A <code>QueriedApplicationDTO</code> that represents the current
    ...
    public QueriedApplicationDTO getApplication(String username, String languageCode) {
        ...
        QueriedApplicationDTO application; 
        try {
            application = (QueriedApplicationDTO) query.getSingleResult();
        } catch (Exception e) {
            return null;
        }
